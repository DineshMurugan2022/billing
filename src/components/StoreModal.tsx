...
export default function StoreModal({ store, onClose, onSave }: any) {
  const [form, setForm] = useState(
    store || { name: '', gstNumber: '', address: '', city: '', state: '', pincode: '', phone: '', email: '' }
  );
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="card w-full max-w-lg mx-4 shadow-2xl animate-slide-up">
        <div className="flex items-center justify-between p-5 border-b border-slate-200">
          <h2 className="font-bold text-lg text-slate-900">{store ? 'Edit Store' : 'Add Store'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="text" className="input" value={form.pincode} onChange={e => setForm({ ...form, pincode: e.target.value })} />
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="btn-secondary flex-1">Cancel</button>
            <button type="submit" className="btn-primary flex-1"><Check size={16} /> Save Store</button>
          </div>
        </form>
      </div>
    </div>
